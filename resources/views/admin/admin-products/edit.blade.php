...
<h2 class="font-heading font-bold text-3xl text-white leading-tight flex items-center">
    ...
    {{ __('Edit Product (NFC Card)') }}
</h2>
<p class="text-indigo-100 mt-2">Update product information</p>
@endsection
...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-purple-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8">
            <form action="{{ route('admin.admin-products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="file" name="image" id="image" accept="image/*"
                           class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                    <p class="mt-2 text-sm text-gray-500">Leave empty to keep current image. Recommended size: 600x400px. Max file size: 2MB</p>
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    @if($product->image && Storage::disk('public')->exists($product->image))
                        <div class="mt-4">
                            ...
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="max-w-full h-64 object-cover rounded-xl border border-gray-300">
                        </div>
                    @endif
                ...
                <div class="mb-6">
                    ...
                    <textarea name="features" id="features" rows="4" placeholder="Enter product features, one per line..."
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">{{ old('features', $product->features) }}</textarea>
                    @error('features')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
